...
sumoCmd = [sumoBinary, "-c",
           "C:\\Users\\user\\Desktop\\sumo\\laneChange\\space-re2\\myConfig.sumocfg"]
# traci.start(sumoCmd)
...
def initPosition():
    ...
    for i in range(11):
        ...
        saftyAll.append(0)
        decArrayAll.append(0)
        changeArrayAll.append(0)
        changeTimeAll.append(0)
        changeFlagAll.append(0)
...
def moveNonFleetPosition(times):
    ...
    print("qqppr :"+str(traci.vehicle.getPosition("no1")[1]))
    firstNoFleetPosition = temp1 - 20 + 530
...
def distributeSpace():
    ...
    xPositionSpaceLast = otherPosition[maxIndex+1]
    xPositionSpaceFleetLast = fleetPosition[10]
    ...
    frontSpeed = traci.vehicle.getSpeed("no"+str(maxIndex))
    print("newSpaceSpeed1: "+str(newSpaceSpeed1))


    firstAnswer = calACCSpeedDiff(locationH1,fleetPosition[0],fleetSpeed[0],newSpaceSpeed1,timeDec,1.5,firstGroupSpaceDec)
    print("firstAnswer: "+str(firstAnswer))


    testAnswer = calRiskBySpeed(fleetSpeed[0],frontSpeed,8,1.5,1)
    if ":" in str(testAnswer):
        characters = "{x:}"
        tempString = str(testAnswer)
        for j in range(len(characters)):
            tempString = tempString.replace(characters[j], "")
        safty = float(tempString)
        if safty < 0:
            groupSafty[0] = fleetSpeed[0] * 1.5
        else:
            groupSafty[0] = safty
    print(testAnswer)
    # groupSafty[0]
    if firstAnswer[0][0] < 0:
        decArray[0] = 0
    else:
        decArray[0] = firstAnswer[0][0]
    # groupSafty[0] = firstAnswer[0][2]
...
def speedDecCounti():
    global startIndex,calculateDecFlag,decArray
    frontSpeed = traci.vehicle.getSpeed("no0")
    # otherPosition
    stopPosition = traci.vehicle.getPosition("stop")[0]

    if calculateDecFlag == 0:  
        for i in range(0,10):
            veh = "veh"+str(i)
            testAnswer = calRiskBySpeed(fleetSpeed[i],frontSpeed,8,1.5,1)
            if ":" in str(testAnswer):
                characters = "{x:}"
                tempString = str(testAnswer)
                for j in range(len(characters)):
                ...
                safty = float(tempString)
                if safty < 0:
                    saftyAll[i] = fleetSpeed[i] * 1.5
                else:
                    saftyAll[i] = safty
                
                # print("1234qqq: "+str(saftyAll))

            # crashTime = (stopPosition - fleetPosition[i]) - 4 / fleetSpeed[i]
            # crashTime = crashTime - 3 - (3) * i
            dec = decArray[0]
            if dec < 0 :
                dec = 0
            elif dec > 8:
            ...
            print(dec)
            decArrayAll[i]  = dec
            # if otherPosition[0] - 4 - saftyAll[i] < fleetPosition[i]: 
            #     decArrayAll[i] = 8
            # else:
            #     decArrayAll[i] = 0

            maxDec = max(decArrayAll)
            for j in range(startIndex,10):
                decArrayAll[j] = maxDec
            calculateDecFlag = 1

    for i in range(0,10):
        veh = "veh"+str(i)
        if fleetSpeed[i] - decArrayAll[i]/10 > 0:
            traci.vehicle.setSpeed(veh, fleetSpeed[i] - decArrayAll[i]/10)
            # print(str(i)+": "+str(fleetSpeed[i]))
        else:
            traci.vehicle.setSpeed(veh, 0)
    ...
    # print("1234qqq: "+str(saftyAll))
        ...
        # if  fleetPosition[i] + 
        # maxSpace = spaceCopy[maxIndex]


# 檢查空間與車隊距離
def spaceFleetClose():
    global step, groupStartChangeArray,recoverSpeedFlag,startIndex

    # frontSpeed = 0
    frontSpeed = traci.vehicle.getSpeed("no0")

    # print("0: "+str(otherPosition[0]))
    # print("1: "+str(fleetPosition[startIndex]))
    # print("2: "+str(saftyAll[startIndex]))
    # print("3: "+str(changeArrayAll[startIndex]))
    # print("4: "+str(saftyAll[startIndex]))

    # print("sddsdsd")
    

    if startIndex == 0:
        if abs(otherPosition[0] - 4 - fleetPosition[startIndex]-saftyAll[startIndex]) < (frontSpeed - fleetSpeed[startIndex]) * 1.5  and changeArrayAll[startIndex] == 0 and saftyAll[startIndex] > 0:
            # recoverSpeedFlag = 1
            decArrayAll[startIndex] = 0
            changeTimeAll[startIndex] = step
            veh = "veh" + str(startIndex)
            traci.vehicle.changeLane(veh, 1, 300)
            # traci.vehicle.setSpeed(veh, 10)
            changeArrayAll[startIndex] = 1
            startIndex = startIndex + 1
    else: 
        if abs(fleetPosition[startIndex - 1] - 4 - 5 - fleetPosition[startIndex]) < (fleetSpeed[startIndex-1] - fleetSpeed[startIndex]) * 1.5 and changeArrayAll[startIndex] == 0 and saftyAll[startIndex] > 0 and changeTimeAll[startIndex-1] > 0 and step > changeTimeAll[startIndex-1]+ 15 :
            # recoverSpeedFlag = 1
            changeTimeAll[startIndex] = step
            decArrayAll[startIndex] = 0
            veh = "veh" + str(startIndex)
        ...
        # if startIndex > 0:
        #     print(fleetPosition[startIndex - 1])
        #     print(fleetPosition[startIndex])
...
def laneChangeSpeedRecover():
    ...
    for i in range(0,10):
        if changeArrayAll[i] == 1 and step >= changeTimeAll[i] + 15:
            if changeFlagAll[i] == 0:
                print("speed"+str(i)+": "+str(fleetSpeed[i]))
                veh = "veh" + str(i)
                # traci.vehicle.setSpeed(veh, frontSpeed)
                if abs(fleetSpeed[i] - target) < 0.6:
                    # if backSpeed == target:
                    changeFlagAll[i] = 1
                    traci.vehicle.setSpeed(veh,target)
                    # groupChangeFlag[i] = 1
                else:
                    traci.vehicle.setSpeed(veh, fleetSpeed[i] + 6 / 10)
    ...
    #         if groupChangeArray[i] == 1 and step >= groupChangeTime[i] + 150:
    ...
    #             frontSpeed = traci.vehicle.getSpeed("no"+str(groupSpaceArray[i]-1))
    #             backSpeed = traci.vehicle.getSpeed("no"+str(groupSpaceArray[i]))
    #             nonChangeCount = 0
    #             if nonChangeFlag[i] != 1 or groupChangeFlag[i] != 1:
    #                 for j in range(groupSpaceArray[i],12):
    #                     noVeh = "no" + str(j)
    #                     noVehSpeed = traci.vehicle.getSpeed(noVeh)
    #                     print("noVehSpeed: "+str(noVehSpeed))
    #                     if step >= groupChangeTime[i] + 300+ ((j+1)*150): 
    #                         if noVehSpeed < fleetPosition[9]:
    #                             traci.vehicle.setSpeed(noVeh, noVehSpeed + 6 / 100)
    #                         else:
    #                             traci.vehicle.setSpeed(noVeh, fleetPosition[9])
    #                             nonChangeCount = nonChangeCount + 1
    #                 if nonChangeCount == 12 and groupChangeFlag[i]==1:
    #                     nonChangeFlag[i] = 1
    #                 nonChangeCount = 0
                        
    #             print("backSpeed: "+str(backSpeed))
    #             if groupChangeFlag[i] == 0:
    #                 for j in range(groupArray[i], 10):
    #                     print("speed"+str(j)+": "+str(fleetSpeed[j]))
    #                     veh = "veh" + str(j)
    #                     # traci.vehicle.setSpeed(veh, frontSpeed)
    #                     if abs(fleetSpeed[j] - target) < 0.05:
    #                         if backSpeed == target:
    #                             groupChangeFlag[i] = 1
    #                             traci.vehicle.setSpeed(veh,target)
    #                         # groupChangeFlag[i] = 1
    ...
    #                 noVehSpeed = traci.vehicle.getSpeed(noVeh)
    #                 print("noVehSpeed: "+str(noVehSpeed))
    #                 if noVehSpeed < target:
    #                     traci.vehicle.setSpeed(noVeh, noVehSpeed + 6 / 100)
    #                 else:
    #                     traci.vehicle.setSpeed(noVeh, target) 
    #             print("backSpeed: "+str(backSpeed))
...
def calculateAllSpaceNeeded():
    global maxSpace, maxIndex, secondMax, secondMaxIndex, h1,h2,hOne
    global firstGroupSpaceDec, firstGroupSpaceSafety,secondGroupSpaceDec,secondGroupSpaceSafety,firstGroupNumber
    ...
    speedI = traci.vehicle.getSpeed("no"+str(maxIndex + 1))
    speedJ = traci.vehicle.getSpeed("no"+str(secondMaxIndex + 1))
    ...
    speedI_front = traci.vehicle.getSpeed("no"+str(maxIndex))
    speedJ_front = traci.vehicle.getSpeed("no"+str(secondMaxIndex))
    ...
    solve_value = solve([
                        hOne-di+eSafe*2+((speedI_front-speedI)*t+0.5*ai*t**2), 
                        # h2-dj+h1+eSafe2*2+((speedJ_front-speedJ)*t+0.5*(ai+aj)*t**2), 
                        # speedI+ai*t-(speedJ+aj*t), 
                        hOne-(10*((((speedI+ai*t)**2-(speedI+ai*t)**2)/(2*8)+(speedI+ai*t)*0.1)+carLenghth)+eSafe*2), 
    ...
    print("firstGroupSpaceDec : "+str(firstGroupSpaceDec))
    hOne = float(splitString[1])
    # secondGroupSpaceDec = float(splitString[1])
    # h2 = float(splitString[2])
    firstGroupSpaceSafety = float(splitString[2])
...
def calACCSpeedDiff(locationX,groupPosition,nowSpeed, frontSpeed,t,notice,spaceDec):
    ...
    solve_value = solve([locationX - groupPosition - eSafe  -((nowSpeed-frontSpeed)*t-(0.5*(x-spaceDec)*(t**2))), 
                        newSpeed-(nowSpeed - x * t), 
                        # eSafe - (((newSpeed**2 - frontSpeed**2) /(2 * 8)) + newSpeed*1.5)
                        # eSafe - newSpeed * notice
                        ((newSpeed**2 * 8) /(frontSpeed**2 +2*8*(eSafe - newSpeed * notice)) / 8) - 1
...
def riskFollowing2():
    ...
    if step > 201 and target - frontSpeed > 0.5 and target - frontSpeed >= 0:
        gap = fleetPosition[9] - otherPosition[1] - 4
        testAnswer = calRiskBySpeed(nowSpeed,frontSpeed,8,notice,1)
        if ":" in str(testAnswer):
            characters = "{x:}"
            tempString = str(testAnswer)
            for j in range(len(characters)):
    ...
    elif step > 201 and  target - frontSpeed < 0.5 and target - frontSpeed >= 0:
        for i in range(1,11):
            now = "no"+str(i)
            if traci.vehicle.getSpeed(now) - target < 0.6:
                oneSpeed = 6 / 10
                newSpeed = traci.vehicle.getSpeed(now) + oneSpeed
                traci.vehicle.setSpeed(now, newSpeed)
...
def startSimulate(speedFleet, times, saveName1, saveName2,saveName3):
    ...
    # while step < 40000:
        ...
        if step > 20:
            ...
            if decStopFlag == 0 and step >= 21:
                # print("fsdfsdf")
                distributeSpace()
                
            if step >= 21:
                speedDecCounti()
            # if decStopFlag == 1 and recoverSpeedFlag == 0:
            #     countiDecGroup()
        ...
        if step == 1:
            for i in range(11):
                ...
                traci.vehicle.setSpeed(noString, target)
                if i == 10:
        ...
        f.write(str(step/10)+save+"\n")
        fileOne.write(str(step/10)+save+"\n")
        save = ""
        for i in range(10):
            vehString = "veh"+str(i)
            ...
            save = save + " " + str(gap)
        f2.write(str(step/10)+save+"\n")
        save = ""
        for i in range(10):
            vehString = "veh"+str(i)
            gap = str(traci.vehicle.getPosition(vehString)[0])
            save = save + " " + str(gap)
        f3.write(str(step/10)+save+"\n")
        save = ""
        for i in range(10):
            vehString = "veh"+str(i)
        ...
        f5.write(str(step/10)+save+"\n")
        fileTwo.write(str(step/10)+save+"\n")
...
def initVal():
    ...
    global groupSpaceArray,groupStartChangeArray
    global groupArray, groupChangeArray, groupChangeTime, groupChangeSpeedUp,groupChangeFlag,groupSafty
    global recoverSpeedFlag

    global decArrayAll,saftyAll,startIndex,calculateDecFlag,changeTimeAll,changeArrayAll,changeFlagAll
    ...
    groupSpaceArray.clear()
    groupStartChangeArray.clear()
