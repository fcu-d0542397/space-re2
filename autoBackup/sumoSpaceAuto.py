...
def speedDecCounti():
    ...
    for i in range(0,10):
        veh = "veh"+str(i)
        if fleetSpeed[i] - decArrayAll[i]/100 > 0:
            traci.vehicle.setSpeed(veh, fleetSpeed[i] - decArrayAll[i]/100)
...
def spaceFleetClose():
    ...
    if startIndex == 0:
        ...
        if abs(fleetPosition[startIndex - 1] - 4 - 5 - fleetPosition[startIndex]) < (fleetSpeed[startIndex-1] - fleetSpeed[startIndex]) * 1.5 and changeArrayAll[startIndex] == 0 and saftyAll[startIndex] > 0 and changeTimeAll[startIndex-1] > 0 and step > changeTimeAll[startIndex-1]+ 150 :
            # recoverSpeedFlag = 1
            changeTimeAll[startIndex] = step
            decArrayAll[startIndex] = 0
            veh = "veh" + str(startIndex)
...
def laneChangeSpeedRecover():
    ...
    for i in range(0,10):
        if changeArrayAll[i] == 1 and step >= changeTimeAll[i] + 150:
            if changeFlagAll[i] == 0:
                print("speed"+str(i)+": "+str(fleetSpeed[i]))
                veh = "veh" + str(i)
                # traci.vehicle.setSpeed(veh, frontSpeed)
                if abs(fleetSpeed[i] - target) < 0.05:
                    # if backSpeed == target:
                    changeFlagAll[i] = 1
                    traci.vehicle.setSpeed(veh,target)
                    # groupChangeFlag[i] = 1
                else:
                    traci.vehicle.setSpeed(veh, fleetSpeed[i] + 6 / 100)
...
def riskFollowing2():
    ...
    if step > 201 and target - frontSpeed > 0.5 and target - frontSpeed >= 0:
        ...
        for i in range(1,11):
            now = "no"+str(i)
            if traci.vehicle.getSpeed(now) - target < 0.05:
                oneSpeed = 6 / 100
                newSpeed = traci.vehicle.getSpeed(now) + oneSpeed
                traci.vehicle.setSpeed(now, newSpeed)
